<?php
	...
	if(isset($_GET['purchase_id'])){
		...
		for($i = 0; $i < sizeof($data); $i++){
			...
			$json_data[$i][0]['data']  = $data[$i] -> purchase_id;
			$json_data[$i][0]['type']  = "text";

			// purchase total
			$json_data[$i][1]['data']  = $data[$i] -> purchase_amount;
			$json_data[$i][1]['type']  = "price";
			$json_data[$i][1]['class'] = "";

			// product status
			$json_data[$i][2]['data']  = $data[$i] -> purchase_status;
			$json_data[$i][2]['value'] = $data[$i] -> purchase_status;
			$json_data[$i][2]['type']  = "badge_radio";

			if($data[$i] -> purchase_status == 1){
				$json_data[$i][2]['class'] = "badge badge-success";
				$json_data[$i][2]['value'] = "SELESAI";
			}
			else{
				$json_data[$i][2]['class'] = "badge badge-danger";
				$json_data[$i][2]['value'] = "BELUM CHECKOUT";
			}

			// purchase type
			$json_data[$i][3]['data']  = $data[$i] -> purchase_type;
			$json_data[$i][3]['type']  = "badge";
			$json_data[$i][3]['class'] = "badge badge-info";

			// purchase date
			$json_data[$i][4]['data']  = $data[$i] -> purchase_date;
	...
	if(isset($_GET['purchase_id'])){
		...
		// table settings
		$json_settings = array('search_index' => 0);
